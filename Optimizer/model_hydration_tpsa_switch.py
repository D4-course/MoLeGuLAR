...
parser.add_argument("--use_wandb", default='no', help="Perform logging using wandb")
...
use_arr = np.array([True, False, False])
for i in range(n_iterations):
    if args.switch == 'yes':
        if args.solvation == 'yes' and args.tpsa == 'yes':
            if i % switch_frequency == 0:
                use_arr = np.roll(use_arr, 1)
                use_docking, use_solvation, use_tpsa = use_arr
